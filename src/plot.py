...
def animate_drone():
    ...
    ani = FuncAnimation(fig, update, frames=num_time_frames, interval=10, blit=True)
    plt.show()
...
def animate_drone_cumulative():
    ...
    def update(frame):
        cov = drone.detection_sphere(frame, terrain, pixel_size)
        cov = np.max(cov, axis=2)  # Use cov here, not coverage.
        # If you intend to accumulate the coverage over frames, update the cumulative coverage.
        # (Make sure to handle variable scoping appropriately.)
        cumulative_coverage = np.maximum(coverage, cov)
    ...
    ani = FuncAnimation(fig, update, frames=num_time_frames, interval=50, blit=True)
    plt.show()
